...
namespace BotCore.Engine.Nodes.EndpointNodes
{
    public class StudentAllLessonsNode(StudentClient client) : ScrollableEndpointNode<LessonDto>("stAllLessons")
    {
        ...
        public override async Task<IEnumerable<LessonDto>> GetEnumerable(DataBuffer input)
        {
            return (await client.LessonsAsync(input.UserId!.Value)).OrderBy(x => x.DateTime);
        }
