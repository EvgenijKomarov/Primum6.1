...
const AbonementCard = ({ abonement, mutateAbonements }: AbonementCardProps) => {
    ...
    const ACTIVATION_CONFIG: Record<AbonementStatus, {popupDescription: string, handler: () => void}> = {
        [AbonementStatus.Active]: { popupDescription: 'Вы уверены, что хотите заморозить абонемент? Расписание останется, но новые занятия создаваться перестанут', handler: hadleFreezeAbonement },
